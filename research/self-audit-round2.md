...
## 14. 審查者錯誤第三例:grep 找字串 ≠ 找概念(2026-08-17)

為 ASP 的 G5 定義 drift 開 issue 時,我在公開 issue 裡寫下:

> 「`GLOSSARY.md` **沒有 G5 詞條**;只在 Reality Checker 條目提到 G5 交叉驗證」

**這是錯的。** `GLOSSARY.md:15` 確實有:

```
| G1-G6 | Quality Gate 1-6 | ADR→SPEC→測試先行→實作→安全→部署 六道品質門 |
```

第 5 個位置寫「安全」。我 grep 的是字串 `G5`,而那一列用 `G1-G6` 表示區間,所以沒命中;
我從「grep 沒命中」推論成「沒有詞條」,並據此宣稱 ADR-033 的描述有誤。
**實際上 ADR-033 是對的,我的「更正」才是錯的。** 已在 issue #98 公開更正。

### 與前兩例的關係

| # | 案例 | 錯誤形式 |
|---|---|---|
| 1 | `anysearch` 的 S-003 | 看到 `.env` 就推論是假陽性,沒去讀 `.ps1` —— **rubric 對、我錯** |
| 2 | slidev-deck-stack PR#1 | 推論它會被新 CI 擋,沒去看 diff —— 它早就 bump 過版本了 |
| 3 | 本例 | grep 沒命中就推論不存在 —— **ADR 對、我錯** |

三次都是同一個形狀:**用一個間接訊號代替直接查證**。
`skill-reviewer/SKILL.md` 步驟 5 已經有第 1 例產出的紀律「複核 = 去查,不是憑印象推翻」;
本例補上一條更尖的變體:

> **搜尋工具沒命中,只證明「這個字串不在那裡」,不證明「這個概念不在那裡」。**
> 區間表示法(`G1-G6`)、同義詞、翻譯、縮寫都會讓字串搜尋漏掉概念。
> 要斷言「某文件沒有某概念」,必須**讀過該文件的相關段落**,不能只出示 grep 的空結果。

這條之所以值得記,是因為它剛好反過來打了本研究自己:整個 rubric 的 script 層
就是一堆 regex,而 regex 的漏判正是我們把 craft 交給 LLM 的理由。
**我在方法論上寫對了,執行時卻犯了同一個錯。**

## 15. 同一錯誤模式的第四次:核對定錨數量時連錯三輪(2026-08-17)

執行 craft 一致性量測時,審查者 R3 揭露「rubric 的 `evidence_refs` 點名了本批 15 個樣本中的 5 個」。
我去核對——**連錯三次**:

| 輪次 | 做法 | 得到 | 錯在哪 |
|---|---|---|---|
| 1 | regex 比對 `full_name` 與 repo `name` | 3 個 | **沒比對 owner**。rubric 寫的是「browser-act」「addyosmani」這種 owner 名 |
| 2 | 加上 owner 比對 | 5 個 | 對了,但下一步又弄壞 |
| 3 | 收緊 lookbehind 為 `(?<![\w/-])` | 又變 3 個 | `/` 進了排除集,於是 `K-Dense/khazix/text-to-cad` 裡的 `text-to-cad` 比不到 |
| 4 | **把所有 `evidence_refs` 直接列出來人工看** | **6 個** | ✅ |

正確答案是 6 個(其中 5 格落在本次評分的 4 個維度內;`text-to-cad` 那筆在 S-101)。
**R3 說 5、R2 說 2,都比我前三輪更接近。**

### 為什麼這條要記

這是 §14 記錄的錯誤模式的**第四次**,而且發生在我**剛寫完那條紀律之後**:

> 搜尋工具沒命中,只證明「這個字串不在那裡」,不證明「這個概念不在那裡」。

§14 寫完的當天我就又犯了三次,只是換了形式——不是「沒命中就當不存在」,
而是「**命中數就當答案**」,沒去確認我的 pattern 涵蓋了所有寫法(owner / name / full_name /
路徑中夾帶)。同一個病根:**把工具輸出當成事實,而不是把它當成一個需要驗證的線索。**

### 收斂出的做法

「某集合有幾個元素符合條件」這類問題,**當集合小到可以列印時就直接列印**。
本例的 `evidence_refs` 只有 8 行,全部印出來人工掃一遍要 10 秒,
而我為了「自動化」寫了三輪 regex、錯了三輪。**自動化的收益要大於它的除錯成本才值得。**

這條也讓 §14 的紀律更完整:寫紀律不會讓你免疫,只有把驗證方式改掉才會。
