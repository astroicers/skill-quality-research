...
**目前 0 條。** 自審輪那一列已依「儀器 bug 即修」先例結案(2.3.15):lint 半邊修畢(evals/tests 下 fixtures 豁免 hygiene+craft 抽樣、**security 刻意不豁免**——路徑豁免=掃描繞道;透明欄位 fixture_skill_md_count;selftest 專案例+突變負向驗證);偵測器文件型 FP 半邊維持「步驟 5 複核承接、不改 regex」的既有裁定,只記樣態。 第六批(2026-09-03,9 條)已清空——**9 條全動手、九刀全條文化**(rubric 3.8.0 / 工具 2.3.14;零核心判別法失效,全為位階/粒度/指涉縫)。全文 [`misjudgment-batch-2026-09-03-sixth.md`](misjudgment-batch-2026-09-03-sixth.md)。文末兩表為處理前列原文,存檔不再更新。 第五批(2026-09-03,9 條)已清空——7 動手(6 條文 + 1 工具/協定)、2 併入動手項,rubric 3.7.0 / 工具 2.3.12;含一次工具突變抓到真縫(多行 evidence_refs 孤兒項)。全文 [`misjudgment-batch-2026-09-03-fifth.md`](misjudgment-batch-2026-09-03-fifth.md)。文末兩表為該批處理前的列原文,存檔不再更新。 批次 4 終審的 F7/F8 已於 2026-09-03 隨熟成輪 2 守衛 PR 迷你批次處理(→3.6.2;偏離門檻理由:終審已驗證+使用者「一併處理」指示,詳 CHANGELOG [2.3.9])。 第四批(2026-09-03,fresh 波 6 條)已清空——六條全動手(rubric 3.6.0 / 工具 2.3.7),含逐案 verdict 穩定性驗算與一次修法自查;全文 [`misjudgment-batch-2026-09-03.md`](misjudgment-batch-2026-09-03.md)。 第三次批次(2026-09-02,15 條)已清空——15 條全數處置(12 全動手、3 部分動手;rubric 3.4.0 / 工具 2.3.3);查證、逐條處置與負向驗證輸出見
...
**rubric370 波蓄積(2026-09-03,作者半 3 條;詳 `review-rubric370-wave-2026-09.md`):**

| 日期 | 對象 | 規則 | 它說什麼 | 我認為應該是什麼 |
|------|------|------|----------|------------------|
| 2026-09-03 | rubric370 波(單源作者;rankup 一案) | L-001 intent_capture × dispatcher | 收編判 mixed 起判;dispatcher 例外只免 L-003 | **「捕獲即路由」無裁定**:總控型 desc 枚舉他人 job 觸發語、body 逐句路由到姊妹 skill——desc 層收編與 body 層路由的抵扣關係缺條文(捕獲後確實交棒 vs 劫持後自吸收,後果不同) |
| 2026-09-03 | rubric370 波(單源;self-improving 一案) | L-001 evaluation_surface × 位置規則 | 非標準觸發欄比照 body;位置規則管文本損壞 | **標準欄因格式損壞而失效**是第三形:frontmatter 前置散文使整塊 frontmatter 可能不被解析,desc 寫得再好載入器讀不到。評估面與位置規則的交界無裁定 |
| 2026-09-03 | rubric370 波(單源;styleseed 44px 一案) | L-004 傘式引用涵蓋面 | 「僅及出處實際涵蓋的主張」 | **涵蓋面存疑時的查證義務未定**:審查者不查外部標準無法裁定涵蓋;本輪臨時處置「記疑不判罪」宜入正文(憑記憶判罪=拿間接訊號當直接證據) |

**rubric370 波 N 定稿蓄積(2026-09-03,6 條;詳 `review-rubric370-wave-2026-09.md` §N friction):**

| 日期 | 對象 | 規則 | 它說什麼 | 我認為應該是什麼 |
|------|------|------|----------|------------------|
| 2026-09-03 | N #1(與作者本輪同惑,雙源) | L-001 pass_criteria × disambiguation | 「無 SEO 堆疊」在高分子句;disambiguation 寫「扣分(同義詞灌水)」 | **堆疊位階矛盾**:純灌水(無收編項)依「基礎唯一出處=pass_criteria」無法降值,而 disambiguation 暗示可扣。需明訂:純灌水=高分面缺陷記 findings,或=基礎違反 |
| 2026-09-03 | N #2(單源,未咬到) | craft_value_mapping L-001~003 聚合 | 全同/any-poor/單 mixed 三句 | **n/a 樣本處置缺**(L-004 的 collection_aggregation 有明文,這裡沒有)——比照補一句 |
| 2026-09-03 | N #3(雙源:N 反覆格 + 作者本輪 borderline) | L-002 mixed_boundary | 「裸 MUST 段」 | **段級 vs 逐條粒度未定**:少數裸核心行散在有理由條目間,兩讀相反值。需判別句(建議:以「核心路徑規則的裸露占比」判段,零星裸行記 findings) |
| 2026-09-03 | N #4(N 裁定已被呼叫端採納) | L-004 傘式引用 × 第三方 API bullet | 引用豁免字面掛在「領域主張」bullet | **傘式與 API/工具 bullet 交互未定**:References 裸外鏈不構成整檔傘(否則 reference 文類憑 footer 免疫)——N 讀法採納,待條文化 |
| 2026-09-03 | N #5(單源;c1 s1 兩讀之源) | L-001 evaluation_surface | description 是唯一輸入 | **skill name 前綴可否作限定詞證據未定**(子命令式集合的域訊號載在名稱上)——需一句(傾向:name 屬觸發輸入的一部分,可作限定詞佐證、不替代情境句) |
| 2026-09-03 | N #6+#7 併記(單源) | L-004 scope_of_perishable / statement_test | 「它無法控制的東西」;「dated snapshot(含日期)」 | **兩處指涉未明**:(a)「它」= shipped skill 檔而非作者組織(N 讀法與「本地產物」裁定同構,採納待條文化);(b) 釘版無日期(版本釘+contract check)落哪格——N 認整體為強機制,條文無明文 |

**自審輪蓄積(2026-09-03;詳 `review-selftools-2026-09-03.md`):**

| 日期 | 對象 | 規則 | 它說什麼 | 我認為應該是什麼 |
|------|------|------|----------|------------------|
| 2026-09-03 | 自審輪(單源雙實例) | lint H-005 / craft_llm_todo 抽樣 / S-001·S-003 | 全部 SKILL.md 一視同仁掃描與抽樣 | **工具開發/偵測器文件形狀無排除**:evals/fixtures 的故意壞夾具入 H-005 並吃掉 craft 抽樣 4/5 格;自述攻擊樣例觸發 S-001/S-003(偵測器打中自己的文件,第三批同型)。修法候選:抽樣與 H-005 對 `evals/fixtures/` 路徑豁免(或降 info);偵測器文件型 FP 已由步驟 5 複核承接、只記樣態不改 regex |
