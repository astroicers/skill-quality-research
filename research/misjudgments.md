...
**目前 6 條(fresh 波蓄積,見文末新表;勘誤後 +4)。** 第三次批次(2026-09-02,15 條)已清空——15 條全數處置(12 全動手、3 部分動手;rubric 3.4.0 / 工具 2.3.3);查證、逐條處置與負向驗證輸出見
...
**fresh 波蓄積(2026-09-02,6 條;詳 `review-fresh-wave-2026-09.md`,後 4 條來自盲判者 K v3 的勘誤輪):**

| 日期 | 對象 | 規則 | 它說什麼 | 我認為應該是什麼 |
|------|------|------|----------|------------------|
| 2026-09-02 | fresh 波(nie 型) | L-003 `domain_lookup_scope` 判別法 | 「這些子題會不會在同一次任務中被同一個人用到?」 | **對「單一框架多應用場景」型失真**——expertise 單一(一套方法論)但應用天然發散(讀新聞 vs 選志願不同任務),expertise 讀 good、任務共現讀 mixed,值搖擺;**三源收斂**:作者判讀、反匿名前哨(自條文側)、盲判者 K(不知情)各自獨立走到同一兩讀且 K 自選 expertise 讀。需一句裁定判別客體 |
| 2026-09-02 | fresh 波(headcount 型) | L-004 `collection_aggregation`「機制存在性不一致」 | 某些樣本有機制、某些沒有 → mixed | **未分兩型**:「有易腐表面而無機制」vs「無表面所以無機制(序4)」——4/5 樣本無表面、唯一有表面者有機制,字面讀 mixed、需求-覆蓋匹配讀 good,**verdict 搖擺(A↔AWN)**;**雙源收斂**:作者與盲判者 K 獨立走到同一兩讀、同選匹配讀。與 wave4 的位置/形式條款同條文第三個未定面 |
| 2026-09-02 | K v3(headcount) | 集合聚合規則**僅 L-004 有** | 任務要求四維皆聚合 | L-001/L-002/L-003 無聚合條文,判讀者被迫移植 L-004 邏輯——L-002 的「4 好 1 壞」因此在 good↔mixed 兩讀,**本輪實際翻了 verdict**(A→NR 的一半成因)。三維各需一句聚合規則 |
| 2026-09-02 | K v3(headcount) | statement_test 對「聲明式紀律」 | 算/不算清單皆未列 | anti-padding(「空欄就明說,不得填充」)+ 強制 Assumptions/falsifier 欄——非先查型也非泛泛免責,**good↔n/a 值域移動**(CE/CHRO 序1↔序4)。需正反清單補一例裁定 |
| 2026-09-02 | K v3(CISO) | craft_value_mapping「傷及基礎 vs 排版」 | 位置規則只例示措辭/排版/邊角 | **結構性文本損壞**(規則句中斷+殘句錯掛+條目重複)落在核心約束清單時算哪類無判別法——mixed↔good 兩讀,本輪實際翻值。需一句:「照字面讀即壞」的損壞不屬排版 |
| 2026-09-02 | v1+K v3(CE/nie) | intent_capture 判別單位 | 針對「裸通用動詞/名詞」 | **寬情境子句與長列舉的統攝範圍未裁定**:孤立讀=收編、整條語境讀=非收編(CE 的 pressure-testing 子句、nie 的高考志愿列舉)——雙源兩讀,nie 的 L-001 三判讀者 2:1 分裂。需裁定判別單位(逐項 vs 整條)與中文列舉統攝文法 |
