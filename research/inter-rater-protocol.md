...
>
> ## 🛑 κ 驗證 rubric 修訂:**不可行,已放棄**(2026-08-18 裁定)
>
> 第二輪的結論不是「樣本不夠」,是**這條路在本領域結構上走不通**。
>
> 兩個條文**完全沒改**的維度變動幅度**大於**我改過的兩個:
> L-001 −0.213、L-003 −0.319 vs L-002 −0.068、L-004 +0.091。
> 用這兩個未改動維度的實測 Δ 反推輪間變異:
>
> ```
> 觀測 |Δκ| 平均 = 0.266  →  SD(κ) ≈ 0.188  (n=14)
> 要解析 0.1 級效果(SD ≈ 0.035)→ 每維度需 n ≈ 404
> 本研究母體 97 repo、rubric 樣本 54       →  差 7.6 倍
> ```
>
> **用光整個母體都還差 7.6 倍。** 這不是預算問題,是這個領域供不出足夠樣本。
> (粗估自兩個觀測值,不是 power calculation;但量級結論不受影響。)
>
> ### 改採:保留機制,丟掉統計量
>
> 兩輪下來**真正有產出的不是 κ,是審查者的 `rubric_friction` 與 `contamination` 筆記**——
> 三位各自獨立讀出 L-004 的邏輯矛盾、L-002 的相反讀法,以及**四類我沒預見的污染源**。
> 那些發現完全不需要 kappa。
>
> **往後的做法**:派 2–3 位審查者讀一批 skill,**只收分歧與卡點筆記,不算 kappa**。
> 不需要 15 個 repo 撐統計效力,幾個形狀夠分散的樣本就有產出,成本降一個量級,
> 而且那是**唯一被證明有效的部分**。
>
> ### 若仍要做 κ 量測,以下四件必須先修
>
> (全部由審查者揭露,不在原隔離清單裡;**第 3、4 項已由
> `scripts/extract_rater_corpus.py` 實作**)
>
...
> 3. ✅ **已實作**——`scripts/extract_rater_corpus.py` 把抽樣到的 SKILL.md 抽到中性目錄,
>    不含 `CLAUDE.md`/`AGENTS.md`/`.claude/rules/`/hooks/scripts。
>    這同時堵掉三位審查者都揭露的 **prompt-injection 面**
>    (harness 會把 untrusted repo 的 CLAUDE.md 當**專案指令**注入;惡意 repo 可據此指示審查者評分)
> 4. ✅ **已實作**——同一支腳本產出的 rater-safe lint 副本已移除 `desc_has_trigger`(L-001 的預判)
...
3. **派 3 位獨立審查者**,每位拿到且只拿到:
   ...
   **必須隔離的東西**(否則量到的是抄襲不是一致性):
   ...
   - `research/directive-polarity.md`(2026-08-18 新增:公開具名 repo 的 craft 評級
     與逐項標記百分比,涵蓋本語料多數 repo)
