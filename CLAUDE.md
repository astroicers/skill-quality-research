...
- **B5 雙判據達成**:判準半邊 verdict-swinging 兩讀=0(3.6.2 六刀髒語料全單讀);
  operator 半邊零程序事故(終局三規則首次全程:前哨先行、靜默期 ≥ 已跑時長、
  編造錨 grep 攔截、盲判修正 land 前吸收——viper AWN→NR、rj L-001 校正)
- **判讀包協定升級**:常規剝除 evidence_refs + vendored 上游名入遮蔽清單
  (滲漏第 2 類實錘;ledger 蓄積待落碼)
- ledger:sqr **3 條蓄積**(集合路由治理位階、跨維主維、vendored 規則化)、
  rr 0;誤判率序列 0.31→0.26→0.50→0.43(抽查輪穩定高於純作者輪,
  縫絕對量遞減且全為聚合/協定層——**核心四維零新縫**)
