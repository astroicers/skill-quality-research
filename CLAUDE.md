...
- **不要再量「指令極性 / 舉例密度」**(2026-08-18,`research/directive-polarity.md`)——
  四個確定性指標 + 一輪 10 repo × 5 盲審全部否證,**rubric 未動、維持 2.1.0**。
  三個要點:①「正向 vs 負向」與「有沒有舉例」是**同一結構的兩個有損側面**
  (一個 `Bad:/Good:` 同時是舉例與極性配對);② 失敗根因是
  **「什麼算一條規則」這個單位不可 regex 化**(blader 用編號標題+四段模板、
  obra 用粗體祈使句、Jeffallan 用 `MUST DO:` 清單——分母錯了,所有比率就都錯了);
  ③ 三位盲審**獨立**收斂到:禁令是否有害**不取決於失效型態,取決於有沒有附替代示範**
  ——而那正是 L-002 的原文
