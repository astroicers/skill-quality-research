...
維度評級中只有 **2 份**含 poor(**4%**)。後果是 **craft verdict 連續 41 個對象全部 `approved`**,
...
現行 4% → `≥2 mixed` 20% → `≥3 mixed` 11%。選 `≥2` 是因為 4% 已被證明等同關閉,
而 20% 對「需要修」是可接受的比率。**⚠️ 該模擬只有 3 個維度**(質化筆記無 L-004 欄),
實際規則有 4 個維度,**真實觸發率會高於 20%**。若實用後發現過度觸發,調整這一格即可
——請把調整依據記進 `research/misjudgments.md`,不要憑感覺改。
